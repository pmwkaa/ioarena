...
__attribute__((unused)) static void dump_bson(const char *bsdata) {
...
static int ia_ejdb_open(const char *datadir) {
  ...
  case IA_SYNC:
    ...
    break;
  case IA_LAZY:
    ia_log("error: %s(): not support syncmode %s", __func__,
  ...
  ia_log("EJDB collection options:\n"
         ...
         "Deflate collection: %s. Collection records will be compressed with "
         "DEFLATE compression. Default: false\n"
         "Expect record number: %d. Expected records number in the collection. "
         "Default: 128K\n"
         "Cahce in memory records: %d. Maximum number of records cached in "
         "memory. Default: 0\n",
  ...
  ia_log("let's create / open collection for openned EJDB  %s\n",
         db_collection_name);
  self->jc =
      ejdbcreatecoll(self->jb, db_collection_name, &db_collection_options);
  if (self->jc == NULL) {
...
void ia_ejdb_thread_dispose(iacontext *ctx) { free(ctx); }
