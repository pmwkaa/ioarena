...
static int ia_wt_open(const char *datadir) {
  ...
  case IA_LAZY:
    durability_config =
        "transaction_sync=(enabled=true,method=none),checkpoint_sync=true";
    break;
  case IA_NOSYNC:
    durability_config =
        "transaction_sync=(enabled=false),checkpoint_sync=false";
    break;
  default:
  ...
  char config[256];
  snprintf(config, sizeof(config), "create,cache_size=1Gb,%s%s", wal_config,
           durability_config);
...
static iacontext *ia_wiredtiger_thread_new(void) {
  ...
  int rc = self->conn->open_session(self->conn, NULL, self->session_config,
                                    &ctx->session);
  if (rc != 0)
    goto bailout;

  rc = ctx->session->open_cursor(ctx->session, self->table_name, NULL,
                                 self->cursor_config, &ctx->cursor);
  if (rc != 0)
    goto bailout;
...
static int ia_wiredtiger_begin(iacontext *ctx, iabenchmark step) {
  ...
  case IA_CRUD:
    rc =
        ctx->session->begin_transaction(ctx->session, self->transaction_config);
    if (rc != 0)
      goto bailout;
    ctx->transaction = 1;
...
iadriver ia_wt = {.name = "wiredtiger",
                  .priv = NULL,
                  .open = ia_wt_open,
                  .close = ia_wt_close,
