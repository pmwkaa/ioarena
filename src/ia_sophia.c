...
static void ia_sophia_on_log(char *trace, void *arg) {
...
static int ia_sophia_open(const char *datadir) {
  ...
  sp_setstring(self->env, "sophia.on_log", (void *)(uintptr_t)ia_sophia_on_log,
               0);
...
bailout:
  ...
  ia_log("error: %s, %s (%d)", __func__, error,
         (int)sp_getint(self->env, "sophia.error"));
  free(error);
  return -1;
}
...
bailout:
  error = sp_getstring(self->env, "sophia.error", 0);
  ia_log("error: %s, %s, %s (%d)", __func__, ia_benchmarkof(step), error,
         (int)sp_getint(self->env, "sophia.error"));
  free(error);
  return -1;
}
...
iadriver ia_sophia = {.name = "sophia",
                      .priv = NULL,
                      .open = ia_sophia_open,
