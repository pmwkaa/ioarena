...
static int ia_forestdb_open(const char *datadir) {
  ...
  self->config.compactor_sleep_duration =
      1; // otherwise compactor won't get time to start
  ...
  case IA_NOSYNC:
    ...
    break;
  default:
    ia_log("error: %s(): unsupported syncmode %s", __FUNCTION__,
  ...
  case IA_WAL_OFF:
    ...
    break;
  default:
    ia_log("error: %s(): unsupported walmode %s", __FUNCTION__,
...
static int ia_forestdb_begin(iacontext *ctx, iabenchmark step) {
  ...
  case IA_ITERATE:
    status = fdb_iterator_init(ctx->kvs, &ctx->it, NULL, 0, NULL, 0,
                               FDB_ITR_NO_DELETES);
    if (status != FDB_RESULT_SUCCESS)
      goto bailout;
...
bailout:
  ia_log("error: %s, %s, %s", __FUNCTION__, ia_benchmarkof(step),
...
iadriver ia_forestdb = {.name = "forestdb",
                        .priv = NULL,
