...
static int ia_leveldb_next(iacontext *ctx, iabenchmark step, iakv *kv) {
  ...
  case IA_SET:
    if (ctx->batch)
      leveldb_writebatch_put(ctx->batch, kv->k, kv->ksize, kv->v, kv->vsize);
    else
      leveldb_put(self->db, self->wopts, kv->k, kv->ksize, kv->v, kv->vsize,
                  &error);
  ...
  case IA_GET:
    if (ctx->result)
      free(ctx->result);
    ctx->result = leveldb_get(self->db, self->ropts, kv->k, kv->ksize,
                              &kv->vsize, &error);
    if (error)
      goto bailout;
    if (!ctx->result) {
...
iadriver ia_leveldb = {.name = "leveldb",
                       .priv = NULL,
