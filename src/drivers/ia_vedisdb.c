...
__attribute__((unused)) static void diag_put(vedis *pStore) {
  const char *zBuf;
  int iLen;
...
data_consumer_callback(const void *pData, unsigned int nDatalen,
...
static int ia_vedisdb_open(const char *datadir) {
  ...
  case IA_LAZY:
  case IA_NOSYNC:
    ia_log("VedisDB: %s(): not support any configurable options for manage "
           "syncmode. Current: %s\n",
...
static int ia_vedisdb_next(iacontext *ctx, iabenchmark step, iakv *kv) {
  ...
  */
  ...
  case IA_SET:
    ...
    if (rc != VEDIS_OK)
      ia_log("error vedis_kv_store, rc = %d\n", rc);
    rc = (rc == VEDIS_OK ? 0 : -1);
    break;
  case IA_DELETE:
  ...
  case IA_GET:
    ...
    // rc =
    // vedis_kv_fetch_callback(self->db,kv->k,kv->ksize,data_consumer_callback,0);
    if (rc == VEDIS_OK) {
...
iadriver ia_vedisdb = {.name = "vedisdb",
                       .priv = NULL,
